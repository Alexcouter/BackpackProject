...
//			System.out.println("test");
	...
	public void creationArbreN2(PileObjet pile){
		...
		if(pile.estVide() > 1){
			...
			if(newSac.getPoids() + o.getPoids() <= newSac.getPoidsMax()){
				this.arbres.add(new ArbreN(newSac));//cr�ation nouveau fils vide
				while(newSac.getPoids() + o.getPoids() <= newSac.getPoidsMax()){//on cr�e un fils a chaque meme objet que l'on peu rajouter
					newSac.addObjet(o);
					this.arbres.add(new ArbreN(newSac.clone()));
				}
	...
	//	System.out.println("test : objet o : " + o.getCout() + " taille arbre " + arbres.size());
		...
		if(this.arbres.size() == 1){
			...
			/*while(iterator.hasPrevious()){ 
			   ArbreN item = iterator.previous();
			   float calcul = calculEsp�rance(pile.top(), item);
	...
	/*
	 * test solution cr�ation arbre courcircuit� total, + utilisation m�thode solutionV2
	 */
